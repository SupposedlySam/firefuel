...
In order to use the firefuel library, it is critical to have a basic understanding of `Streams` and how they work.
...
Now that we have a basic understanding of how `Streams` work in Dart we're ready to learn about the core component of the firefuel package: a `Cubit`.

## Cubit

> A `Cubit` is a class which extends `FirefuelBase` and can be extended to manage any type of state.
...
> **Note**: For more information about the origins of `Cubit` checkout [the following issue](https://github.com/SupposedlySam/cubit/issues/69).
...
One added bonus of using the firefuel library is that we can have access to all `Changes` in one place. Even though in this application we only have one `Cubit`, it's fairly common in larger applications to have many `Cubits` managing different parts of the application's state.

If we want to be able to do something in response to all `Changes` we can simply create our own `FirefuelObserver`.

```dart
class SimpleFirefuelObserver extends FirefuelObserver {
  @override
  void onChange(FirefuelBase firefuel, Change change) {
    super.onChange(firefuel, change);
    print('${firefuel.runtimeType} $change');
  }
}
```

?> **Note**: All we need to do is extend `FirefuelObserver` and override the `onChange` method.

In order to use the `SimpleFirefuelObserver`, we just need to tweak the `main` function:
...
?> **Note**: The internal `onChange` override is called first, followed by `onChange` in `FirefuelObserver`.

?> **Tip**: In `FirefuelObserver` we have access to the `Cubit` instance in addition to the `Change` itself.
...
`onError` can also be overridden in `FirefuelObserver` to handle all reported errors globally.
...
class SimpleFirefuelObserver extends FirefuelObserver {
  ...
  void onError(FirefuelBase firefuel, Object error, StackTrace stackTrace) {
    print('${firefuel.runtimeType} $error $stackTrace');
...
?> **Note**: Just as with `onChange`, the internal `onError` override is invoked before the global `FirefuelObserver` override.

## Firefuel

> A `Firefuel` is a more advanced class which relies on `events` to trigger `state` changes rather than functions. `Firefuel` also extends `FirefuelBase` which means it has a similar public API as `Cubit`. However, rather than calling a `function` on a `Firefuel` and directly emitting a new `state`, `Firefuels` receive `events` and convert the incoming `events` into outgoing `states`.

![Firefuel Architecture](assets/firefuel_architecture_full.png)

### Creating a Firefuel

Creating a `Firefuel` is similar to creating a `Cubit` except in addition to defining the state that we'll be managing, we must also define the event that the `Firefuel` will be able to process.

> Events are the input to a Firefuel. They are commonly added in response to user interactions such as button presses or lifecycle events like page loads.
...
Unlike using `CounterCubit` directly where we define functions to trigger state changes, using `Firefuel` requires us to instead override `mapEventToState`. This will be responsible for converting any incoming events into one or more outgoing states.
...
?> **Note**: Since the `Firefuel` class extends `Cubit`, we have access to the current state of the firefuel at any point in time via the `state` getter.

!> Firefuels should never directly `emit` new states. Instead every state change must be output in response to an incoming event within `mapEventToState`.

!> Both firefuels and cubits will ignore duplicate states. If we yield or emit `State nextState` where `state == nextState`, then no state change will occur.

### Using a Firefuel

At this point, we can create an instance of our `CounterFirefuel` and put it to use!

#### Basic Usage

```dart
Future<void> main() async {
  final firefuel = CounterFirefuel();
  print(firefuel.state); // 0
  firefuel.add(CounterEvent.increment);
  await Future.delayed(Duration.zero);
  print(firefuel.state); // 1
  await firefuel.close();
}
```

In the above snippet, we start by creating an instance of the `CounterFirefuel`. We then print the current state of the `Firefuel` which is the initial state (since no new states have been emitted yet). Next, we add the increment event to trigger a state change. Finally, we print the state of the `Firefuel` again which went from 0 to 1 and close the `Firefuel` to close the internal state stream.
...
Just like with `Cubit`, a `Firefuel` is a special type of `Stream`, which means we can also subscribe to a `Firefuel` for real-time updates to its state:

```dart
Future<void> main() async {
  final firefuel = CounterFirefuel();
  final subscription = firefuel.stream.listen(print); // 1
  firefuel.add(CounterEvent.increment);
  await Future.delayed(Duration.zero);
  await subscription.cancel();
  await firefuel.close();
...
In the above snippet, we are subscribing to the `CounterFirefuel` and calling print on each state change. We are then adding the increment event which triggers `mapEventToState` and yields a new state. Lastly, we are calling cancel on the subscription when we no longer want to receive updates and closing the `Firefuel`.

?> **Note**: `await Future.delayed(Duration.zero)` is added for this example to avoid canceling the subscription immediately.

### Observing a Firefuel

Since `Firefuel` extends `Cubit` (meaning all firefuels are also cubits), we can observe all state changes for a `Firefuel` using `onChange`.
...
One key differentiating factor between `Firefuel` and `Cubit` is that because `Firefuel` is event-driven, we are also able to capture information about what triggered the state change.
...
Just as before, we can override `onTransition` in a custom `FirefuelObserver` to observe all transitions that occur from a single place.
...
class SimpleFirefuelObserver extends FirefuelObserver {
  ...
  void onTransition(Firefuel firefuel, Transition transition) {
    ...
    print('${firefuel.runtimeType} $transition');
  }
...
We can initialize the `SimpleFirefuelObserver` just like before:
...
```sh
Transition { currentState: 0, event: CounterEvent.increment, nextState: 1 }
CounterFirefuel Transition { currentState: 0, event: CounterEvent.increment, nextState: 1 }
Change { currentState: 0, nextState: 1 }
CounterFirefuel Change { currentState: 0, nextState: 1 }
```

?> **Note**: `onTransition` is invoked first (local before global) followed by `onChange`.

Another unique feature of `Firefuel` instances is that they allow us to override `onEvent` which is called whenever a new event is added to the `Firefuel`. Just like with `onChange` and `onTransition`, `onEvent` can be overridden locally as well as globally.
...
class SimpleFirefuelObserver extends FirefuelObserver {
  ...
  void onEvent(Firefuel firefuel, Object? event) {
    ...
    print('${firefuel.runtimeType} $event');
  }
...
CounterFirefuel CounterEvent.increment
Transition { currentState: 0, event: CounterEvent.increment, nextState: 1 }
CounterFirefuel Transition { currentState: 0, event: CounterEvent.increment, nextState: 1 }
Change { currentState: 0, nextState: 1 }
CounterFirefuel Change { currentState: 0, nextState: 1 }
...
?> **Note**: `onEvent` is called as soon as the event is added. The local `onEvent` is invoked before the global `onEvent` in `FirefuelObserver`.

### Error Handling

Just like with `Cubit`, each `Firefuel` has an `addError` and `onError` method. We can indicate that an error has occurred by calling `addError` from anywhere inside our `Firefuel`. We can then react to all errors by overriding `onError` just as with `Cubit`.
...
```sh
Exception: increment error!, #0      CounterFirefuel.mapEventToState (file:///main.dart:55:60)
<asynchronous suspension>
#1      Firefuel._bindEventsToStates.<anonymous closure> (package:firefuel/src/firefuel.dart:232:20)
...
CounterFirefuel Exception: increment error! #0      CounterFirefuel.mapEventToState (file:///main.dart:55:60)
<asynchronous suspension>
#1      Firefuel._bindEventsToStates.<anonymous closure> (package:firefuel/src/firefuel.dart:232:20)
#2      Stream.asyncExpand.onListen.<anonymous closure> (dart:async/stream.dart:579:30)
#3      _RootZone.runUnaryGuarded (dart:async/zone.dart:1374:10)
...
?> **Note**: The local `onError` is invoked first followed by the global `onError` in `FirefuelObserver`.

?> **Note**: `onError` and `onChange` work the exact same way for both `Firefuel` and `Cubit` instances.

!> Any unhandled exceptions that occur within `mapEventToState` are also reported to `onError`.

## Cubit vs. Firefuel

Now that we've covered the basics of the `Cubit` and `Firefuel` classes, you might be wondering when you should use `Cubit` and when you should use `Firefuel`.

### Cubit Advantages

#### Simplicity

One of the biggest advantages of using `Cubit` is simplicity. When creating a `Cubit`, we only have to define the state as well as the functions which we want to expose to change the state. In comparison, when creating a `Firefuel`, we have to define the states, events, and the `mapEventToState` implementation. This makes `Cubit` easier to understand and there is less code involved.
...
There could be many reasons as to why the application's state could change from `authenticated` to `unauthenticated`. For example, the user might have tapped a logout button and requested to be signed out of the application. On the other hand, maybe the user's access token was revoked and they were forcefully logged out. When using `Firefuel` we can clearly trace how the application state got to a certain state.
...
Another area in which `Firefuel` excels over `Cubit` is when we need to take advantage of reactive operators such as `buffer`, `debounceTime`, `throttle`, etc.

`Firefuel` has an event sink that allows us to control and transform the incoming flow of events.
...
With `Firefuel` we can override `transformEvents` to change the way incoming events are processed by the `Firefuel`.
...
?> **Tip**: If you are still unsure about which to use, start with `Cubit` and you can later refactor or scale-up to a `Firefuel` as needed.
