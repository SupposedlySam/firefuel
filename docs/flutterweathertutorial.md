...
- Persist state across sessions ([HydratedFirefuel](https://github.com/SupposedlySam/firefuel/tree/main/packages/hydrated_firefuel))

## Key Concepts

- Observe state changes with [FirefuelObserver](/coreconcepts?id=firefuelobserver)
- [FirefuelProvider](/flutterfirefuelcoreconcepts?id=firefuelprovider), Flutter widget that provides a firefuel to its children
- [FirefuelBuilder](/flutterfirefuelcoreconcepts?id=firefuelbuilder), Flutter widget that handles building the widget in response to new states
- Prevent unnecessary rebuilds with [Equatable](/faqs?id=when-to-use-equatable)
- [RepositoryProvider](/flutterfirefuelcoreconcepts?id=repositoryprovider), a Flutter widget that provides a repository to its children
- [FirefuelListener](/flutterfirefuelcoreconcepts?id=firefuellistener), a Flutter widget that invokes the listener code in response to state changes in the firefuel
- [MultiFirefuelProvider](/flutterfirefuelcoreconcepts?id=multifirefuelprovider), a Flutter widget that merges multiple FirefuelProvider widgets into one
- [FirefuelConsumer](/flutterfirefuelcoreconcepts?id=firefuelconsumer), a Flutter widget that exposes a builder and listener in order to react to new states
- [HydratedFirefuel](https://github.com/SupposedlySam/firefuel/tree/main/packages/hydrated_firefuel) to manage and persist state
...
> Following the [firefuel architecture](https://firefuellibrary.dev/#/architecture) guidelines, our application will consist of several layers.
...
[models.dart](https://raw.githubusercontent.com/SupposedlySam/firefuel/main/examples/flutter_weather/packages/meta_weather_api/lib/src/models/models.dart ':include')
...
[location.dart](https://raw.githubusercontent.com/SupposedlySam/firefuel/main/examples/flutter_weather/packages/meta_weather_api/lib/src/models/location.dart ':include')

#### Weather Model

Here is our complete `weather.dart` model file:

[weather.dart](https://raw.githubusercontent.com/SupposedlySam/firefuel/main/examples/flutter_weather/packages/meta_weather_api/lib/src/models/weather.dart ':include')
...
[meta_weather_api_client.dart](https://raw.githubusercontent.com/SupposedlySam/firefuel/main/examples/flutter_weather/packages/meta_weather_api/lib/src/meta_weather_api_client.dart ':include')

#### Barrel File Updates

Let's wrap up this package by adding our API client to the barrel file.

[meta_weather_api.dart](https://raw.githubusercontent.com/SupposedlySam/firefuel/main/examples/flutter_weather/packages/meta_weather_api/lib/meta_weather_api.dart ':include')
...
[location_test.dart](https://raw.githubusercontent.com/SupposedlySam/firefuel/main/examples/flutter_weather/packages/meta_weather_api/test/location_test.dart ':include')

#### Weather Tests

[weather_test.dart](https://raw.githubusercontent.com/SupposedlySam/firefuel/main/examples/flutter_weather/packages/meta_weather_api/test/weather_test.dart ':include')
...
?> **Note**: We don't want our tests to make real API calls since our goal is to test the API client logic (including all edge cases) and not the API itself. In order to have a consistent, controlled test environment, we will use [mocktail](https://github.com/SupposedlySam/mocktail) to mock the `http` client.

[pubspec.yaml](_snippets/flutter_weather_tutorial/data_layer/mocktail_pubspec.yaml.md ':include')

[meta_weather_api_client_test.dart](https://raw.githubusercontent.com/SupposedlySam/firefuel/main/examples/flutter_weather/packages/meta_weather_api/test/meta_weather_api_client_test.dart ':include')
...
[pubspec.yaml](https://raw.githubusercontent.com/SupposedlySam/firefuel/main/examples/flutter_weather/packages/weather_repository/pubspec.yaml ':include')
...
[weather.dart](https://raw.githubusercontent.com/SupposedlySam/firefuel/main/examples/flutter_weather/packages/weather_repository/lib/src/models/weather.dart ':include')

Update the barrel file we created previously to include the models.

[models.dart](https://raw.githubusercontent.com/SupposedlySam/firefuel/main/examples/flutter_weather/packages/weather_repository/lib/src/models/models.dart ':include')
...
[weather_repository.dart](https://raw.githubusercontent.com/SupposedlySam/firefuel/main/examples/flutter_weather/packages/weather_repository/lib/src/weather_repository.dart ':include')

#### Barrel File

Update the barrel file we created previously.

[weather_repository.dart](https://raw.githubusercontent.com/SupposedlySam/firefuel/main/examples/flutter_weather/packages/weather_repository/lib/weather_repository.dart ':include')

### Unit Tests

> Just as with the data layer, it's critical to test the repository layer in order to make sure the domain level logic is correct. To test our `WeatherRepository`, we will use the [mocktail](https://github.com/SupposedlySam/mocktail) library. We will mock the underlying api client in order to unit test the `WeatherRepository` logic in an isolated, controlled environment.

[weather_repository_test.dart](https://raw.githubusercontent.com/SupposedlySam/firefuel/main/examples/flutter_weather/packages/weather_repository/test/weather_repository_test.dart ':include')
...
[weather.dart](https://raw.githubusercontent.com/SupposedlySam/firefuel/main/examples/flutter_weather/lib/weather/models/weather.dart ':include')
...
[models.dart](https://raw.githubusercontent.com/SupposedlySam/firefuel/main/examples/flutter_weather/lib/weather/models/models.dart ':include')
...
Using the [Firefuel VSCode](https://marketplace.visualstudio.com/items?itemName=FelixAngelov.firefuel) or [Firefuel IntelliJ](https://plugins.jetbrains.com/plugin/12129-firefuel) extension, right click on the `weather` directory and create a new cubit called `Weather`. The project structure should look like this:
...
[weather_state.dart](https://raw.githubusercontent.com/SupposedlySam/firefuel/main/examples/flutter_weather/lib/weather/cubit/weather_state.dart ':include')
...
[weather_cubit.dart](https://raw.githubusercontent.com/SupposedlySam/firefuel/main/examples/flutter_weather/lib/weather/cubit/weather_cubit.dart ':include')
...
[theme_cubit.dart](https://raw.githubusercontent.com/SupposedlySam/firefuel/main/examples/flutter_weather/lib/theme/cubit/theme_cubit.dart ':include')

### Unit Tests

> Similar to the data and repository layers, it's critical to unit test the business logic layer to ensure that the feature-level logic behaves as we expect. We will be relying on the [firefuel_test](https://pub.dev/packages/firefuel_test) in addition to `mocktail` and `test`.

Let's add the `test`, `firefuel_test`, and `mocktail` packages to the `dev_dependencies`.

[pubspec.yaml.dart](https://raw.githubusercontent.com/SupposedlySam/firefuel/main/examples/flutter_weather/pubspec.yaml ':include')

?> **Note**: The `firefuel_test` package allows us to easily prepare our firefuels for testing, handle state changes, and check results in a consistent way.

#### Theme Cubit Tests

[theme_cubit_test.dart](https://raw.githubusercontent.com/SupposedlySam/firefuel/main/examples/flutter_weather/test/theme/cubit/theme_cubit_test.dart ':include')

#### Weather Cubit Tests

[weather_cubit_test.dart](https://raw.githubusercontent.com/SupposedlySam/firefuel/main/examples/flutter_weather/test/weather/cubit/weather_cubit_test.dart ':include')

## Presentation Layer

### Weather Page

We will start with the `WeatherPage` which uses `FirefuelProvider` in order to provide an instance of the `WeatherCubit` to the widget tree.

[weather_page.dart](https://raw.githubusercontent.com/SupposedlySam/firefuel/main/examples/flutter_weather/lib/weather/view/weather_page.dart ':include')
...
[settings_page.dart](https://raw.githubusercontent.com/SupposedlySam/firefuel/main/examples/flutter_weather/lib/settings/view/settings_page.dart ':include')
...
[search_page.dart](https://raw.githubusercontent.com/SupposedlySam/firefuel/main/examples/flutter_weather/lib/search/view/search_page.dart ':include')
...
[weather_empty.dart](https://raw.githubusercontent.com/SupposedlySam/firefuel/main/examples/flutter_weather/lib/weather/widgets/weather_empty.dart ':include')

#### WeatherError

This screen will display if there is an error.

[weather_error.dart](https://raw.githubusercontent.com/SupposedlySam/firefuel/main/examples/flutter_weather/lib/weather/widgets/weather_error.dart ':include')

#### WeatherLoading

This screen will display as the application fetches the data.

[weather_loading.dart](https://raw.githubusercontent.com/SupposedlySam/firefuel/main/examples/flutter_weather/lib/weather/widgets/weather_loading.dart ':include')

#### WeatherPopulated

This screen will display after the user has selected a city and we have retrieved the data.

[weather_populated.dart](https://raw.githubusercontent.com/SupposedlySam/firefuel/main/examples/flutter_weather/lib/weather/widgets/weather_populated.dart ':include')

### Barrel File

Let's add these states to a barrel file to clean up our imports.

[widgets.dart](https://raw.githubusercontent.com/SupposedlySam/firefuel/main/examples/flutter_weather/lib/weather/widgets/widgets.dart ':include')

### Entrypoint

Our `main.dart` file should initialize our `WeatherApp` and `FirefuelObserver` (for debugging purposes), as well as setup our `HydratedStorage` to persist state across sessions.

[main.dart](https://raw.githubusercontent.com/SupposedlySam/firefuel/main/examples/flutter_weather/lib/main.dart ':include')

Our `app.dart` widget will handle building the `WeatherPage` view we previously created and use `FirefuelProvider` to inject our `ThemeCubit` which handles theme data.

[app.dart](https://raw.githubusercontent.com/SupposedlySam/firefuel/main/examples/flutter_weather/lib/app.dart ':include')

### Widget Tests

The `firefuel_test` library also exposes `MockFirefuels` and `MockCubits` which make it easy to test UI. We can mock the states of the various cubits and ensure that the UI reacts correctly.

[weather_page_test.dart](https://raw.githubusercontent.com/SupposedlySam/firefuel/main/examples/flutter_weather/test/weather/view/weather_page_test.dart ':include')
...
The full source code for this example, including unit and widget tests, can be found [here](https://github.com/SupposedlySam/firefuel/tree/main/examples/flutter_weather).
